...
const sports = [
  "Aquatics", "Athletics", "Powerlifting", "Cricket", 
  "Football", "Volleyball", "Basketball", "Squash", 
  "Table Tennis", "Kabbadi", "Carrom", "Chess", 
  "Lawn Tennis", "Badminton", "Tig of War"
];
  ...
  return (
    <div className="py-24 bg-brand-surface min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <SectionHeading 
          title="Events" 
          subtitle="15+ competitive events testing strength, endurance, and strategy."
        />
          ...
          {sports.map((sport, idx) => (
            <div key={idx} className="group relative rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all h-64 bg-brand-dark cursor-pointer transform hover:-translate-y-1">
              <div className="absolute inset-0 opacity-40 group-hover:opacity-20 transition-opacity">
                 <PlaceholderImage text="" gradient="bg-gradient-to-tr from-brand-dark to-brand-primary" className="w-full h-full" />
              </div>
              <div className="absolute inset-0 bg-gradient-to-t from-brand-dark via-brand-dark/50 to-transparent"></div>
              
              <div className="absolute inset-0 p-6 flex flex-col justify-end">
                <h3 className="text-2xl font-bold text-white mb-2 group-hover:text-brand-highlight transition-colors">{sport}</h3>
